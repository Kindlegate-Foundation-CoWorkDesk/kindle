const Section3 = ({ programs, heading, backgroundC , textColor}) => {
  ...
  return (
    ...
    <div  style={{ backgroundColor: backgroundC, color : textColor }} className="mb-0 pb-10" >
      <div className="p-10" ><h1 className=" mb-10 text-center font-extrabold text-3xl" id='All_Progs'  >{heading}</h1></div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {programs.map((program, index) => (
          <div key={index} className="p-4 md:p-0 flex  rounded-xl" style={{ backgroundColor: program.bg_color }}>
            <div className="w-1/2" id={program.id}>
